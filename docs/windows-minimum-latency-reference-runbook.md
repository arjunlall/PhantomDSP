...
>
> Restore `Speaker Virtualization.txt` as the only renderer include before staging anything. Run `git diff --check`, commit only `measurements/minimum-latency/legacy-reference/raw` plus any strictly necessary tooling fix in a separate commit, and push `codex/dsp-improvements`. Do not run the macOS analyzer. Finish with routing evidence, clipping status, output paths, exact commands, and commit hashes.
