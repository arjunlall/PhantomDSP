...
Candidate J uses I as its exact parent and applies one additional broad, non-iteratively-fitted correction per speaker. It fades in from 900 Hz, normalizes the remaining 1–1.5 kHz complete-to-direct jump to I's established level, then uses a short frequency-shaped release to unity by 1.8 kHz. J differs from I by 0.0164 dB RMS at 200 Hz–1 kHz and 0.0013 dB RMS at 1.8–8 kHz; modeled correlated gain is +2.61 dB. Offline tests pass. Informal listening found J tonally successful, but the image may be too high.

Full-spectrum component analysis shows that J's rising complete-to-direct ratio above 5 kHz is synthetic early energy filling the personal direct-path notch near 7–8 kHz. The explicit floor/ceiling paths are 24–31 dB below direct there; the dominant microclusters are 6–8 dB above it. A 150-subject ARI laboratory-HRTF screen selected five subjects whose ±30° contrast matches the four personal direct paths within 0.88–1.12 dB RMS.

Candidate K now changes only that microcluster branch. It assigns 80% of diffuse directional energy to the horizontal side field and 10% each to floor and ceiling, then redistributes each speaker's existing smoothed microcluster power between its two ear paths using the matched HRTF ensemble. K changes individual localization-band paths by up to roughly 3.3 dB while preserving fused speaker response within 0.11 dB RMS, bass within 0.00004 dB RMS, 200 Hz–1.8 kHz within 0.008 dB RMS, and modeled correlated gain at +2.59 dB. J's WAV hashes remain unchanged. Exact public inputs and fast/full rebuild procedures are in the [HRTF reproduction guide](hrtf-reproduction.md). Informal listening found that K lowered the apparent image slightly and made the stereo stage and phantom image more convincing than J, despite its small fused-response delta.

The remaining timbre audit found that K's inherited microcluster spectrum effectively modeled directional HRTF change without also applying the theoretical monitor directivity and treated-surface losses. Moore–Glasberg auditory-band integration places K's 6–10 kHz microcluster energy 11.57/11.30 dB above the full theoretical left/right targets and 5.98/5.76 dB above a half-strength sensitivity model. The actual complete-response contribution is smaller but still broad: approximately +3.78/+4.22 dB from 7–10 kHz. The [timbre audit](../measurements/synthetic-reference-room/directional-diffuse/timbre-analysis/report.md) therefore warrants one branch-only correction.

The treatment-aware timbre candidate applies attenuation only to K's deterministic microclusters, using one shared minimum-phase filter for both ear paths from each speaker. It leaves direct sound, bass, specular and late fields, timing, and K's directional ratios unchanged. Offline, it matches the theoretical microcluster envelope within 0.01 dB from 6–10 kHz, lowers the complete response by 3.10/3.33 dB on average in that band, changes 3–5 kHz by only 0.03/0.02 dB RMS, and models +2.62 dB maximum correlated gain. Next: Windows Benchmark and direct K/timbre-balanced listening for tonal naturalness without loss of K's spatial improvement.
