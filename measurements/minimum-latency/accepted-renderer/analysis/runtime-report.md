...
The runtime capture passes the digital gate: routing, timing, headroom, CPU load, generated-asset closure, and the legacy-reference tonal match all validate. The 80–200 Hz RMS error is 0.83–1.17 dB and the worst smoothed point is 2.47 dB. Controlled listening subsequently found no readily audible tonal or spatial regression from the legacy reference, while finger drumming confirmed the latency improvement. This is the accepted Speaker Virtualization renderer; the legacy renderer remains the fallback.
