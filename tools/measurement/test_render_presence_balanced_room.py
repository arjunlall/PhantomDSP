...
from render_presence_balanced_room import (
    ANALYSIS_DIRECTORY,
    DESIGN,
    K_FILES,
    L_FILES,
...
class PresenceBalancedRoomTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_design_changes_only_l_correction_onset(self):
        self.assertEqual(DESIGN["base_candidate"], "L treatment-aware timbre candidate")
        self.assertEqual(DESIGN["correction_support_hz"], [5500.0, 14000.0])
        self.assertEqual(DESIGN["correction_full_strength_hz"], [7500.0, 12000.0])
    ...
    def test_outputs_match_summary_and_record_k_and_l_parents(self):
        for side, filename in OUTPUT_FILES.items():
            ...
            self.assertEqual(
                sha256(L_FILES[side]),
                self.summary["candidate_l_files"][side]["sha256"],
            )

    def test_direct_arrival_and_protected_bands_are_unchanged(self):
        ...
        for speaker in ("left", "right"):
            delta = self.summary["response_delta_presence_minus_k"][speaker]
    ...
    def test_presence_is_restored_without_reintroducing_upper_sharpness(self):
        for speaker in ("left", "right"):
            versus_l = self.summary["response_delta_presence_minus_l"][speaker]
            self.assertGreater(versus_l["5.5-6.5 kHz"]["mean_db"], 0.2)
            self.assertLess(versus_l["5.5-6.5 kHz"]["mean_db"], 1.0)
            self.assertGreater(versus_l["6.5-7.5 kHz"]["mean_db"], 0.05)
            self.assertLess(versus_l["6.5-7.5 kHz"]["mean_db"], 0.8)
            self.assertLess(versus_l["7.5-9 kHz"]["rms_db"], 0.05)
            self.assertLess(versus_l["9-11 kHz"]["rms_db"], 0.01)

            versus_k = self.summary["diagnostic_band_delta_presence_minus_k"][speaker]
            self.assertLess(versus_k["7.5-9 kHz"]["mean_db"], -3.0)
            self.assertLess(versus_k["9-11 kHz"]["mean_db"], -2.0)
            self.assertEqual(
