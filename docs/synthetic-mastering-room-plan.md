...
Offline, M restores 0.44/0.52 dB from 5.5–6.5 kHz and 0.14/0.28 dB from 6.5–7.5 kHz relative to L for the left/right virtual speakers. From 7.5–9 kHz it differs from L by less than 0.012 dB RMS, so it does not broadly split the K/L difference. Modeled correlated gain remains +2.63 dB. Windows Benchmark passed with no clipping or configuration errors, 5.13 dB correlated-sweep headroom, and 0.61–0.67% single-core CPU. The [Candidate M report](../measurements/synthetic-reference-room/presence-balanced/analysis/report.md) records the exact K/L/M comparison.
