...
if (($ActiveSelections.Count -ne 2) -or
    ($ActiveSelections[0].Trim() -ne $ExpectedPrototype) -or
    ($ActiveSelections[1].Trim() -ne $ExpectedDefault)) {
    throw "Expected the reserved D200 route followed by the A100 playback default in $SelectorPath."
}

$DeviceName = "$BaseDeviceName PhantomDSP D200 Prototype"
Write-Host "Capturing the opt-in D200 prototype: $DeviceName"
& $CaptureScript `
